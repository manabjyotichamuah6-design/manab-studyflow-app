...
export const AuthView: React.FC<AuthViewProps> = ({
  ...
  const handleEmailSignUp = (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setLoadingMessage('Creating your personal study account...');
  ...
  const handleForgotPassword = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setSuccessMsg(`A password reset link has been dispatched to ${cleanEmail}. Check your inbox.`);
    }, 600);
  };

  return (
    <div className="min-h-[85vh] flex items-center justify-center py-10 px-4 sm:px-6 animate-fade-in">
      <div className="w-full max-w-md bg-[#FAF8F5] rounded-3xl border border-[#E8E4D9] shadow-xl p-6 sm:p-8 text-[#1C1E1B] relative">
        {/* Brand Icon & Heading */}
        ...
        {/* Back to Home / Landing option */}
        {onNavigateToLanding && (
          <div className="mt-6 pt-4 border-t border-[#E8E4D9] text-center">
            <button
              type="button"
              onClick={onNavigateToLanding}
              className="text-xs font-semibold text-[#6B7267] hover:text-[#1B4332] transition-colors"
            >
              ← Back to StudyFlow Overview
            </button>
          </div>
        )}
      </div>
